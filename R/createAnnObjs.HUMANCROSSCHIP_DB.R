...
### HUMANCROSSCHIP_DB-based package i.e. any SQLite-based ann data package based
### on the HUMANCROSSCHIP_DB schema.
### The createAnnObjs.HUMANCROSSCHIP_DB() function is the main entry point for
### this file: it is called by any HUMANCROSSCHIP_DB-based package at load-time.
### -------------------------------------------------------------------------


HUMANCROSSCHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id")
HUMANCROSSCHIP_DB_L2Rlink2 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id", altDB="org.Hs.eg")

### Mandatory fields: objName, Class and L2Rchain
HUMANCROSSCHIP_DB_AnnDbBimap_seeds <- list(
    ##ACCNUM mapping will have to be inserted/stored in the original package DB (no way to store this one in the org package)
    list(
        objName="ACCNUM",
        Class="AnnDbBimap",
        L2Rchain=list(
    ...
    list(
        ...
        Class="AnnDbBimap",
        L2Rchain=list(
            HUMANCROSSCHIP_DB_L2Rlink1,
            HUMANCROSSCHIP_DB_L2Rlink2,
            list(
                tablename="alias",
                Lcolname="_id",
...
createAnnObjs.HUMANCROSSCHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
##     checkDBSCHEMA(dbconn, "HUMANCROSSCHIP_DB")
    ...
    ann_objs <- createAnnDbBimaps(HUMANCROSSCHIP_DB_AnnDbBimap_seeds, seed0)
