...
### BOVINECHIP_DB-based package i.e. any SQLite-based ann data package based
### on the BOVINECHIP_DB schema.
### The createAnnObjs.BOVINECHIP_DB() function is the main entry point for
### this file: it is called by any BOVINECHIP_DB-based package at load-time.
### -------------------------------------------------------------------------

orgPkg = "org.Bt.eg"

BOVINECHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id", filter="{is_multiple}='0'")
BOVINECHIP_DB_L2Rlink2 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id", altDB=orgPkg)

### Mandatory fields: objName, Class and L2Rchain
BOVINECHIP_DB_AnnDbBimap_seeds <- list(
    list(
        objName="ACCNUM",
        Class="AnnDbBimap",
        L2Rchain=list(
...
createAnnObjs.BOVINECHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "BOVINECHIP_DB")
    ...
    ann_objs <- createAnnDbBimaps(BOVINECHIP_DB_AnnDbBimap_seeds, seed0)
    ...
    ann_objs$CHRLENGTHS <- createCHRLENGTHS(dbconn, dbname= "org.Bt.eg")
    ann_objs$MAPCOUNTS <- createMAPCOUNTS(dbconn, prefix)

    ## 1 special string to let us know who the supporting org package is.
    ann_objs$ORGPKG <- "org.Bt.eg"
