...
### CHICKENCHIP_DB-based package i.e. any SQLite-based ann data package based
### on the CHICKENCHIP_DB schema.
### The createAnnObjs.CHICKENCHIP_DB() function is the main entry point for
### this file: it is called by any CHICKENCHIP_DB-based package at load-time.
### -------------------------------------------------------------------------

orgPkg = "org.Gg.eg"

CHICKENCHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id", filter="{is_multiple}='0'")
CHICKENCHIP_DB_L2Rlink2 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id", altDB=orgPkg)

### Mandatory fields: objName, Class and L2Rchain
CHICKENCHIP_DB_AnnDbBimap_seeds <- list(
    list(
        objName="ACCNUM",
...
createAnnObjs.CHICKENCHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "CHICKENCHIP_DB")
    ...
    ann_objs <- createAnnDbBimaps(CHICKENCHIP_DB_AnnDbBimap_seeds, seed0)
    ...
    ann_objs$CHRLENGTHS <- createCHRLENGTHS(dbconn, dbname="org.Gg.eg")
    ann_objs$MAPCOUNTS <- createMAPCOUNTS(dbconn, prefix)
    ## 1 special string to let us know who the supporting org package is.
    ann_objs$ORGPKG <- "org.Gg.eg"
