...
### INPARANOIDDROME_DB-based package i.e. any SQLite-based ann data package
### based on the INPARANOIDDROME_DB schema.
### The createAnnObjs.INPARANOIDDROME_DB() function is the main entry point
### for this file: it is called by any INPARANOIDDROME_DB-based package at
### load-time.
### -------------------------------------------------------------------------
...
createAnnObjs.INPARANOIDDROME_DB <- function(prefix, objTarget, dbconn, datacache)
{
    ...
    INPARANOIDDROME_DB_AnnDbBimap_seeds <- makeSeedList(species, fields)

    checkDBSCHEMA(dbconn, "INPARANOIDDROME_DB")
    ...
    ann_objs <- createAnnDbBimaps(INPARANOIDDROME_DB_AnnDbBimap_seeds, seed0)
