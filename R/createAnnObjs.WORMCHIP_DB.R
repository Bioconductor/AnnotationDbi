...
### WORMCHIP_DB-based package i.e. any SQLite-based ann data package based
### on the WORMCHIP_DB schema.
### The createAnnObjs.WORMCHIP_DB() function is the main entry point for
### this file: it is called by any WORMCHIP_DB-based package at load-time.
### -------------------------------------------------------------------------

orgPkg = "org.Ce.eg"

WORMCHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id", filter="{is_multiple}='0'")
WORMCHIP_DB_L2Rlink2 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id", altDB=orgPkg)

### Mandatory fields: objName, Class and L2Rchain
WORMCHIP_DB_AnnDbBimap_seeds <- list(
    list(
        objName="ACCNUM",
...
createAnnObjs.WORMCHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "WORMCHIP_DB")
    ...
    ann_objs <- createAnnDbBimaps(WORMCHIP_DB_AnnDbBimap_seeds, seed0)
    ...
    ann_objs$CHRLENGTHS <- createCHRLENGTHS(dbconn, dbname="org.Ce.eg")
    ann_objs$MAPCOUNTS <- createMAPCOUNTS(dbconn, prefix)
    ## 1 special string to let us know who the supporting org package is.
    ann_objs$ORGPKG <- "org.Ce.eg"
