...
### INPARANOIDRATNO_DB-based package i.e. any SQLite-based ann data package
### based on the INPARANOIDRATNO_DB schema.
### The createAnnObjs.INPARANOIDRATNO_DB() function is the main entry point
### for this file: it is called by any INPARANOIDRATNO_DB-based package at
### load-time.
### -------------------------------------------------------------------------
...
createAnnObjs.INPARANOIDRATNO_DB <- function(prefix, objTarget, dbconn, datacache)
{
    ...
    INPARANOIDRATNO_DB_AnnDbBimap_seeds <- makeSeedList(species, fields)

    checkDBSCHEMA(dbconn, "INPARANOIDRATNO_DB")
    ...
    ann_objs <- createAnnDbBimaps(INPARANOIDRATNO_DB_AnnDbBimap_seeds, seed0)
