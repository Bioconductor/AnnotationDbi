...
### MOUSECHIP_DB-based package i.e. any SQLite-based ann data package based
### on the MOUSECHIP_DB schema.
### The createAnnObjs.MOUSECHIP_DB() function is the main entry point for
### this file: it is called by any MOUSECHIP_DB-based package at load-time.
### -------------------------------------------------------------------------

orgPkg = "org.Mm.eg"

MOUSECHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id", filter="{is_multiple}='0'")
...
createAnnObjs.MOUSECHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "MOUSECHIP_DB")
    ...
    ann_objs$MGI2PROBE <- revmap(ann_objs$MGI, objName="MGI2PROBE")
