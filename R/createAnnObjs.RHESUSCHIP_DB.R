...
### RATCHIP_DB-based package i.e. any SQLite-based ann data package based
### on the RATCHIP_DB schema.
### The createAnnObjs.RATCHIP_DB() function is the main entry point for
### this file: it is called by any RATCHIP_DB-based package at load-time.
### -------------------------------------------------------------------------

orgPkg = "org.Mmu.eg"

RHESUSCHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id", filter="{is_multiple}='0'")
RHESUSCHIP_DB_L2Rlink2 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id", altDB=orgPkg)

### Mandatory fields: objName, Class and L2Rchain
RHESUSCHIP_DB_AnnDbBimap_seeds <- list(
    list(
        objName="ACCNUM",
    ...
    list(
        ...
        Class="ProbeAnnDbBimap",
        L2Rchain=list(
            RHESUSCHIP_DB_L2Rlink1,
            RHESUSCHIP_DB_L2Rlink2,
            list(
                tablename="alias",
                Lcolname="_id",
...
createAnnObjs.RHESUSCHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "RHESUSCHIP_DB")
    ...
    ann_objs <- createAnnDbBimaps(RHESUSCHIP_DB_AnnDbBimap_seeds, seed0)
    ...
    ann_objs$CHRLENGTHS <- createCHRLENGTHS(dbconn, dbname="org.Mmu.eg")
    ann_objs$MAPCOUNTS <- createMAPCOUNTS(dbconn, prefix)
    ## 1 special string to let us know who the supporting org package is.
    ann_objs$ORGPKG <- "org.Mmu.eg"
