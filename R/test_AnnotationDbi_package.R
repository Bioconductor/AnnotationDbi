.test <- function(dir, pattern = ".*_test\\.R$")
{
    ...
    if (missing(dir)) {
        dir <- system.file("unitTests", package="AnnotationDbi")
        if (!nzchar(dir)) {
            dir <- system.file("UnitTests", package="AnnotationDbi")
            if (!nzchar(dir))
    ...
    suite <- defineTestSuite(name="AnnotationDbi RUnit Tests", dirs=getwd(),
    ...
    if (length(details <- .failure_details(result)) >0) {
        ...
        cat("\n\n")
        stop("unit tests failed for package AnnotationDbi")
    }
    result
}
