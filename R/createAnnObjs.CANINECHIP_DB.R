...
CANINECHIP_DB_L2Rlink1 <- list(tablename="probes", Lcolname="probe_id", Rcolname="gene_id", filter="{is_multiple}='0'")
CANINECHIP_DB_L2Rlink2 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id", altDB=orgPkg)
...
createAnnObjs.CANINECHIP_DB <- function(prefix, objTarget, dbconn, datacache)
{
    ...
    ann_objs$CHRLENGTHS <- createCHRLENGTHS(dbconn, dbname="org.Cf.eg")
    ann_objs$MAPCOUNTS <- createMAPCOUNTS(dbconn, prefix)
    ## 1 special string to let us know who the supporting org package is.
    ann_objs$ORGPKG <- "org.Cf.eg"
