...
### ZEBRAFISH_DB-based package i.e. any SQLite-based ann data package based
### on the ZEBRAFISH_DB schema.
### The createAnnObjs.ZEBRAFISH_DB() function is the main entry point for
### this file: it is called by any ZEBRAFISH_DB-based package at load-time.
### -------------------------------------------------------------------------


ZEBRAFISH_DB_L2Rlink1 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id")

### Mandatory fields: objName, Class and L2Rchain
ZEBRAFISH_DB_AnnDbBimap_seeds <- list(
    list(
        objName="ACCNUM",
        Class="AnnDbBimap",
        L2Rchain=list(
            ZEBRAFISH_DB_L2Rlink1,
            list(
...
createAnnObjs.ZEBRAFISH_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "ZEBRAFISH_DB")
    ...
    ann_objs <- createAnnDbBimaps(ZEBRAFISH_DB_AnnDbBimap_seeds, seed0)
