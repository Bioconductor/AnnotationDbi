...
### ECOLI_DB-based package i.e. any SQLite-based ann data package based
### on the ECOLI_DB schema.
### The createAnnObjs.ECOLI_DB() function is the main entry point for
### this file: it is called by any ECOLI_DB-based package at load-time.
### -------------------------------------------------------------------------


ECOLI_DB_L2Rlink1 <- list(tablename="genes", Lcolname="gene_id", Rcolname="_id")

### Mandatory fields: objName, Class and L2Rchain
ECOLI_DB_AnnDbBimap_seeds <- list(
    list(
        objName="ACCNUM",
        Class="AnnDbBimap",
        L2Rchain=list(
...
createAnnObjs.ECOLI_DB <- function(prefix, objTarget, dbconn, datacache)
{
    checkDBSCHEMA(dbconn, "ECOLI_DB")
    ...
    ann_objs <- createAnnDbBimaps(ECOLI_DB_AnnDbBimap_seeds, seed0)
