...
experimenting with new generic functions \Rfunction{toTable} and
\Rfunction{toList} that provide some additional flexibility in
...
x
toTable(x)[1:6, ]
toTable(x, "40781_at")[1:10, ]

toList(x, "40781_at")   # equivalent to mget("40781_at", x)
@ 
...
This doesn't work (only 2 args to \Rfunction{toList}, x and names):
...
toList(hgu95av2PFAM, "1000_at")
@ 
  4 cols:
<<fourcols>>=
toTable(hgu95av2GO)[1:6, ]    
toList(hgu95av2GO, "1000_at")[[1]][1:2]
@ 
