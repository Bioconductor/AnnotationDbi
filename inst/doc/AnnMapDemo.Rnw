...
\Rclass{AnnMap} objects like \Rclass{environment} instances.  For
...
In addition, we are experimenting with new generic functions
\Rfunction{toTable} and \Rfunction{toList} that provide some
...
\Rclass{AnnMap} objects.  In general, converting to lists will not be
...
<<show.revmap>>=
system.time(sym2p <- revmap(hgu95av2db::hgu95av2SYMBOL))

unlist(mget(syms, revmap(hgu95av2db::hgu95av2SYMBOL)))
@ 
...
zz <- dbGetQuery(hgu95av2db::db_conn, sql)

@ 

A low-level interface is available to access the data in table
(\Rclass{data.frame}) format using \Rfunction{toTable}.

<<toTable>>=
toTable(hgu95av2db::hgu95av2GO, probes[1:3])

@ 


<<thisworks>>=
as.list(revmap(hgu95av2db::hgu95av2PATH), "00300")
@ 
...
<<revmap2>>=
x <- hgu95av2db::hgu95av2PATH
## except for the name, this is exactly revmap(x)
revx <- hgu95av2db::hgu95av2PATH2PROBE
...
There can be more than 2 columns in the returned data frame:
  ...
  3 cols:
<<threecols>>=
toTable(hgu95av2db::hgu95av2PFAM)[1:6, ]  # the right values are tagged
as.list(hgu95av2db::hgu95av2PFAM, "1000_at")
@ 
...
This helps to understand the naming of Rfunction{toTable} args:
<<naming, eval=FALSE>>=
toTable(x, left.names, right.names)
@ 
