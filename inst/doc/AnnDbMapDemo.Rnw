%\VignetteIndexEntry{AnnDbMap API Demo}
...
number of \Rclass{AnnDbMap} objects in place of the \Rclass{environment}
...
\Rclass{AnnDbMap} objects like \Rclass{environment} instances.  For
...
\Rclass{AnnDbMap} objects.  In general, converting to lists will not be
...
zz <- dbGetQuery(hgu95av2.db::db_conn, sql)
})
...
NB: the left keys are always on the left (1st col), the right keys
...
junk <- left.keys(x)    # same for all maps in hgu95av2.db (except pseudo-map
                         # MAPCOUNTS)
left.length(x)           # nb of left keys
junk <- right.keys(x)   # KEGG ids for PATH/PATH2PROBE maps, GO ids for
...
<<naming, eval=FALSE>>=
toTable(x, left.keys, right.keys)
@ 
